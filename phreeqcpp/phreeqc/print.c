...
#include <assert.h>
	...
	static char const svnid[] = "$Id: print.c 5214 2011-03-18 20:30:17Z dlpark $";
...
//#else /* if PHREEQ98 execute punch_user_graph first, that is, before punch.in and pr.punch is checked */
	if (state == TRANSPORT || state == PHAST || state == VS2DRT || state == ADVECTION)
	{
		use.kinetics_ptr = kinetics_bsearch(use.n_kinetics_user, &i);
	}
	else if (use.kinetics_in != FALSE)
 ...
 */
		if (state != TRANSPORT && state != PHAST && state != VS2DRT)
		{
 ...
 */
	...
	{
		if (state == TRANSPORT || state == PHAST || state == ADVECTION || state == VS2DRT)
		{
 ...
 */
	...
	else if (state == PHAST || state == VS2DRT)
	{
		output_msg(OUTPUT_MESSAGE, "\tTime:      %g seconds\n",
 ...
 */
	...
	if (pr.use == FALSE || pr.all == FALSE)
		return (OK);
	if (state < REACTION || phast == TRUE || vs2drt == TRUE)
		return (OK);
/*
 *   Mixture or Solution
 */
...
#define VS2DRT_NULL(x)	  (vs2drt ? NULL : x)
/* ---------------------------------------------------------------------- */
int CLASS_QUALIFIER
 ...
 */
	...
	if (punch.time == TRUE)
	{
		...
		else if (state == TRANSPORT || state == PHAST || state == VS2DRT)
		{
