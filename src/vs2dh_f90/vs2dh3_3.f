      ...
      filen = 'vs2dh.fil'
      JSTOP = 0
C     SET UP TO READ FROM DATA FILE GENERATED BY VS2DHI
...
C          DIMENSIONAL FLUID FLOW AND HEAT TRANSPORT UNDER
...
c      include 'c_scon.inc'
      ...
      COMMON/TRXY/EPS1,EPS2,TRANS,TRANS1,SORP,SSTATE,MB9(72),NMB9
      ...
      CHARACTER*80 TITL,filen,f5,f6,f7,f8,f9,f10,f11,f12,f13,blank
...
c      include 'c_obst.inc'
      ...
      WRITE (06,4140) tunit,(PLTIM(K),K=1,NPLT)
      else
      include 'd_obst.inc'
      END IF
      IF(F11P) THEN
...
C
 ...
 4030 FORMAT(A80/21HMONITORING POINT FILE/2X,6HTIME,  ,A4,2X,
     &5x,'NODE',3x,
     &  6H XR,  ,A4,2X,6H Z,    ,A4,2X,6H H,   ,A4,2X,6H  P,  ,A4,
     & 2X,6H THETA,4X,8H     SAT,5x,'      VX',5x,'      VZ'
     &5x,'      ET')
 4031 FORMAT(A80/21HMONITORING POINT FILE/7X,6HTIME,  ,A4,7X,
     &5x,'NODE',3x,
 ...
 4032 FORMAT(A80/21HMONITORING POINT FILE/2X,6HTIME,  ,A4,2X,
     &5x,'NODE',3x,
     &  6H XR,  ,A4,2X,6H Z,    ,A4,2X,6H H,   ,A4,2X,6H  P,  ,A4,
     & 2X,6H THETA,4X,8H     SAT,9x,'TEMP',5x,'      VX',5x,
     &'      VZ', 5x,'      ET')
 4033 FORMAT(A80/21HMONITORING POINT FILE/7X,6HTIME,  ,A4,7X,
     &5x,'NODE',3x,
     &  6H XR,  ,A4,10X,6H Z,    ,A4,10X,6H H,   ,A4,10X,6H  P,  ,A4,
 ...
 4060 FORMAT(35X,60('+')/35X,'+',26X,6H VS2DH,26X,'+'/35X,
     &'+',23x,' VERSION 3.3',23x,'+'/35x,
     &'+',11X,'SIMULATION OF 2-DIMENSIONAL VARIABLY',11X,'+'/
     &35X,'+',12X,'SATURATED FLOW AND ENERGY TRANSPORT',11X,'+'
 ...
 4130 FORMAT(50X,47HGRID SPACING IN HORIZONTAL OR RADIAL DIRECTION,
     &,3H IN,1X,A4/(10F10.3))
 4140 FORMAT(5X,'TIMES (IN ',a4,') AT WHICH H WILL BE WRITTEN TO ',
     &'FILE 08'/10(5X,10E12.5,/))
 ...
 4190 FORMAT(5X,'TOTAL NUMBER OF ITERATIONS FOR FLOW EQUATION = ',I6
     ...
     &/5x,'TOTAL NUMBER OF ITERATIONS FOR OUTER LOOP = ',I6)
...
C
      ...
      NITT=NITT+NIT
c     END IF
c      IF((TRANS.OR.VPNT).AND..NOT.SSTATE) CALL VTVELO
...
C
      ...
      IF (.NOT.VER1P0) READ (05,*) hydraulicFunctionType,adsorptionType
      ELSE
      READ (05,*) NTEX,NPROP
      IF (.NOT.VER1P0) READ (05,*) hydraulicFunctionType
      NPROP1=0
      END IF
c
...
C
      WRITE (6,4000) EPS,ZUNIT,EPS1,HMAX,EPS2
...
C
   ...
   20 HT(J22,J23)=0.0D0
      ...
      IF(TRANS) THEN
c
c  modification to read in only nontrivial vs2dh parameters
c
      read(5,*) ht(j,1),ht(j,2),ht(j,5),ht(j,9),ht(j,10),ht(j,11)
      write(6,4130) ht(j,1),ht(j,2),ht(j,5),ht(j,9),ht(j,10),ht(j,11)
      ht(j,5)=ht(j,5)*(1.-hk(j,3))
      END IF
c
...
c     convert alpha parameter if reading version 2.5 data set
      IF (VER1P0.AND.(hydraulicFunctionType.EQ.1)) THEN
         DO 31 J=1,NTEX
            IF (HK(J,4).NE.0) THEN
               HK(J,4) = -1.D0/HK(J,4)  
            END IF
...
C
  ...
  180 CONTINUE
C
C    READ INITIAL TEMPERATURES IF TRANSPORT EQUATION IS TO
C    BE SOLVED
C
...
c
 ...
 4000 FORMAT(10X,27HINITIAL MOISTURE PARAMETERS/10X,27(1H_)//
     ...
     &5X,'CONVERGENCE CRITERION FOR SIP FOR TRANSPORT (EPS1) =',1PE12.3,
     &1X,/5X,23HDAMPING FACTOR, HMAX = ,1PE12.3,1X,/5X,
     &'CONVERGENCE CRITERION FOR OUTER ITERATION (EPS2)  = ',1PE12.3)
 ...
 4100 FORMAT(41X,35HCONSTANTS FOR SOIL TEXTURAL CLASSES//
     ...
     &4x,'HK(4)',8X,'HK(5)',7X,'HK(6)',/,36X,7HSTORAGE)
...
C
     ...
     *TUNIT,TRED
      READ (05,*) DSMAX,STERR
      READ (05,*) POND
      WRITE (06,4020) DSMAX,ZUNIT,STERR,ZUNIT,POND,ZUNIT
      READ (05,*) PRNT
...
C
   ...
   50 CONTINUE
      ...
      CS(IN)=CF
c
c  change made 99-12-16 so that ntc = 2 conductive heat flux is
c   in terms of energy per time per length of boundary (i.e.,
c   a line source) , as 
c   opposed to energy per time (which is for point source)
c
      if (ntc.eq.2) then
       area = dxr(nn)
...
c
 ...
 4020 FORMAT(
     ...
     &F8.3,1X,A4,/15X,'STEADY-STATE CLOSURE CRITERION = ',1PE14.5,1X,
     &A4/15X,'MAXIMUM DEPTH OF PONDING = ',1PE14.5,1X,A4)
 4030 FORMAT(1X,'******** WARNING --- NODE TYPE OF 0 ASSIGNED TO ',
     &'BOUNDARY NODE ',2I5)
c
...
C
     ...
     1(Q(N)+QQ(N))*RHO(N)-(A(N)*P(IM1)+B(N)
...
c
      ...
      END IF
      PTMP=P(IN)+Z1
      HCND(IN)=VSHKUTAB(PTMP,CC(IN),J1)
...
c
      ...
      HCND(IN)=VSHKUOT(THETA(IN),CC(IN),J1)
c
c  end revisions
c
      END IF
...
c      bltemp2=VOL*(
c      IF(NCTYP(IN).NE.1) BL(68)=BL(68)+VOL*(
c     *CC(IN)*(HT(JJ,5)+THETA(IN)*RHO(IN)*HT(JJ,11)*(2.0d0+
c     *SS*(P(IN)-PXXX(IN))))-cc(in)*thlst(in)*rho(in)*ht(jj,11)
c     *-COLD(IN)*(theta(IN)*RHOOLD(IN)*HT(JJ,11)+ht(jj,5)))
...
c*****************************
      bltemp2=VOL*(
     *CC(IN)*(HT(JJ,5)+THETA(IN)*RHO(IN)*HT(JJ,11)*(1.0d0+
     *SS*(P(IN)-PXXX(IN))))-cold(in)*(thlst(in)*rho(in)*ht(jj,11)
     *+ht(jj,5)))
      bl(68) = bl(68) + bltemp2
      if (jflag1.eq.1) then
       if(ntyp(in).eq.1) bcmt = bcmt + cc(in)*(theta(in) - thlst(in))
     &*RHOOLD(IN)*HT(JJ,11)*vol
       if(nctyp(in).eq.1) bcmt = bcmt + (theta(in)*RHOOLD(IN)*HT(JJ,11)
     &+ht(jj,5))*(cc(in) - cold(in))*vol
      end if
      if(bltemp2.ge.0.0D0) then
...
C     SS=-HT(JJ,4)*(THETA(IN)+THETA(IN)*P(IN)*SS+RET(IN))*DELT
      ...
      BLTEMP=BLTEMP-RET(IN)*CC(IN)*VOL
...
C
      ...
      IF(TRANS) BL(45)=BL(45)+QQ(IN)*CC(IN)*RHO(IN)*HT(JJ,11)
      ELSE
      BL(9)=BL(9)+QQ(IN)*RHO(IN)
      IF(TRANS) BL(42)=BL(42)+QQ(IN)*CS(IN)*RHO(IN)*HT(JJ,11)
      END IF
      ELSE
C
...
c      END IF
      ...
      IF(QX.LT.0.0D0) THEN
      BL(3)=BL(3)-QX
      IF(TRANS) BL(36)=BL(36)-QX*CS(IN)*HT(JJ,11)
      ELSE
      BL(6)=BL(6)-QX
      IF(TRANS) BL(39)=BL(39)-QX*CC(IN)*HT(JJ,11)
      END IF
      ELSE
C
...
c      IF(TRANS) BL(60)=BL(60)+Q(IN)*CC(IN)*
c     *RHO(IN)*HT(JJ,11)
      END IF
      END IF
c
c  the following was moved from vtsetup, so that AO, etc
c   would not be updated until a final solution had been
c   reached
c
...
c
c******************
c      if(jflag1.ne.1.or.ntyp(IN).ne.1) then
c      SS=THETA(IN)+SS-THLST(IN)
c      end if
c*****************
      EO(IN)=E(IN)+VOL*(SS1+((THETA(IN)+SS)*RHO(IN)*HT(JJ,11)+HT(JJ,5)
     1+RET(IN))/DELT)
      IF(JFLAG1.EQ.1) THEN
        AO(IN)=0.5D0*AO(IN)
        BO(IN)=0.5D0*BO(IN)
...
C
 ...
 4010 FORMAT(21X,10(1H-),1X,'MASS BALANCE SUMMARY FOR TIME STEP',
     ...
     &//2X,128('+')/2X,'+',126X,'+'/
     &2X,'+',90X,' TOTAL THIS',11X,'RATE THIS',5X,'+'/2X,'+',
     &33X,'VOLUMETRIC FLOW BALANCE',
     &18X,'TOTAL   ',9X,'TIME STEP',11X,'TIME STEP',6X,'+'/
 ...
 4011 FORMAT('     TIME      BNDY    WATER      WATER      WATER',
     1'       HEAT       HEAT       HEAT'/,15X,'FACE     FLUX',
 ...
 4030 FORMAT(2X,'+',126X,'+',/,2X,'+',35X,'ENERGY MASS BALANCE',
     ...
     &2X,'+',38X,'TOTAL EVAPOTRANSPIRATION',
     &1X,'--  ',2(1PE15.5,5X),1PE15.5,4X,'+'/
     &2X,'+',45X,'FIRST ORDER DECAY --  ',2(1PE15.5,5X),
     &1PE15.5,4X,'+'/
     &2X,'+',39X,'ADSORPTION/ION EXCHANGE --  ',2(1PE15.5,5X),
     &1PE15.5,4X,'+'/
...
C
c      IF(F7P) THEN
c      WRITE(07,4040) KTIM,STIM,TUNIT,NIT,NIT1,NIT3
c      WRITE(07,4030) (DHMX(M2),M2=1,NIT)
c      END IF
      WRITE(06,4040) KTIM,STIM,TUNIT,NIT,NIT1,NIT3
...
C
 ...
 40   continue
C
C  WRITE TEMPERATURES TO FILE 8
C
      IF(TRANS) THEN
      DO 50 J=1,NLY
      do 45 N=1,NXR
...
C
      ...
      if(nxr.gt.3) then
       WRITE(06,4100) ZUNIT,TUNIT
       CALL VSOUT(1,VX)
      end if
      if(nly.gt.3) then
       WRITE(06,4110) ZUNIT,TUNIT
       CALL VSOUT(1,VZ)
      end if
      END IF
C
...
C
 ...
 4030 FORMAT(7E11.4)
 4040 FORMAT(' TIME STEP ',I9,'  TIME = ',E14.6,1X,A4
     &,'  NIT = ',I5,'  NIT1 = ',I5,'  NIT3 = ',I5)
 4050 FORMAT(6X,A80/5X,20HTOTAL ELAPSED TIME =,1PE14.6,1X,A4
     &,/5X,10HTIME STEP ,I9,//)
 4060 FORMAT(1H ,50X,'TOTAL HEAD, IN ',A4)
 4070 FORMAT(1H ,50X,'PRESSURE HEAD, IN ', A4)
 4080 FORMAT(1H ,50X,10HSATURATION)
 4090 FORMAT(1H ,50X,16HMOISTURE CONTENT)
 4100 FORMAT(51X,'X-VELOCITY, IN ',A4,'/',A4)
 4110 FORMAT(51X,'Z-VELOCITY, IN ',A4,'/',A4)
 4120 FORMAT(51X,'TEMPERATURE, IN DEGREES C')
      END
...
c           Genuchten alpha from original definition used in VS2DH.
...
c      ALPH=HK(I,4)
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION VSHKUVG(P,CC,I)
C*****
CVSHKUVG
C*****
C
...
c      include 'c_rprop.inc'
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION VSHKUBC(P,CC,I)
C*****
CVSHKUBC
C*****
C
...
c      include 'c_rprop.inc'
     ...
     &(CC+273.160D0-140.0D0)))
      ...
      IF(P.GE.HK(I,4))RETURN
      VSHKUBC=VSHKUBC*(HK(I,4)/P)**(2.0D0+3.0D0*HK(I,6))
...
c      include 'c_rprop.inc'
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION VSHKUHK(P,CC,I)
C*****
CVSHKUHK
C*****
C
...
c      include 'c_rprop.inc'
     ...
     &(CC+273.160D0-140.0D0)))
      ...
      IF(P.GE.0.0D0)RETURN
      VSHKUHK=VSHKUHK/((P/HK(I,4))**HK(I,6)+1.0D0)
      RETURN
      END
C **********************************************************************
C **********************************************************************
...
c      I4=I4-1
   ...
   3  CONTINUE
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION VSHKUTAB (P,CC,I)
C*****
CVSHKUTAB
C*****
C
...
c      include 'c_rprop.inc'
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION VSHKUOT(P,CC,I)
C*****
CVSHKUOT
C*****
C
     ...
     &(CC+273.160D0-140.0D0)))
      ...
      if (p.ge.hk(i,3)) then
       ...
       if (p.lt.hk(i,12)) then
        ...
        vshkuot = vshkuot*r1**(0.50D0)*xi3**(2.0D0)/hk(i,13)
       else
...
c      RHO1=RHO(N)
      RHO(N)=VTRHO(CC(N),N2)
c      RHO2=DABS(RHO1-RHO(N))
c      IF(RHO2.GT.RHOMAX)RHOMAX=RHO2
c      RET(N)=VTRET(CC(N),N2)
      ret(n)=0.0D0
      IM1=N-NLY
      JM1=N-1
      JP1=N+1
...
C
     ...
     1-DX1(IP1)*(RHO(N)+RHO(IP1))-DZ1(JP1)*(RHO(N)+RHO(JP1))
...
C#
      END IF
c      IF(Q(N).LT.0.) E(N)=E(N)+Q(N)*RHO(N)
...
c      END IF
      E(N)=E(N)-VOL*(SS1+((THETA(N)+SS)*RHO(N)*HT(N2,11)+HT(N2,5)
     1+RET(N))/DELT)
      end if
c
...
C
C     RHS(N)=-VOL*(THETA(N)*(1+P(N)*HK(N2,2)/HK(N2,3))+RET(N))*COLD(N)/
C#    RHS(N)=-VOL*(THETA(N)*RHO(N)*HT(N2,11)+HT(N2,5)+RET(N))*COLD(N)/
...
c
c*************************
c      RHS(N)=RHS(N)-VOL*(THETA(N)*RHO(N)*HT(N2,11)+HT(N2,5)+RET(N))
      if (ntyp(n).eq.1) then
      RHS(N)=RHS(N)-VOL*(THETA(N)*RHO(N)*HT(N2,11)+HT(N2,5)+RET(N))
     &*COLD(N)/DELT
     &-A(N)*CC(IM1)-B(N)*CC(JM1)
     &-C(N)*CC(IP1)-D(N)*CC(JP1)-E(N)*CC(N)
      else
      RHS(N)=RHS(N)-VOL*(THLST(N)*RHO(N)*HT(N2,11)+HT(N2,5)+RET(N))
     &*COLD(N)/DELT
     &-A(N)*CC(IM1)-B(N)*CC(JM1)
     &-C(N)*CC(IP1)-D(N)*CC(JP1)-E(N)*CC(N)
      end if
     ...
     **COLD(JM1)-CO(N)*COLD(IP1)-DO(N)*COLD(JP1)-EO(N)*COLD(N)
     ...
     &*RHO(N)*HT(N2,11)
      IF(QT(N).LT.0.0D0.AND.NCTYP(N).EQ.0) then
       if(cit.and.jflag1.ne.1) then
        RHS(N)=RHS(N)+0.5d0*(dum(n)+QT(N))*CS(N)*HT(N2,11)
       else
        rhs(n) = rhs(n) + qt(n)*cs(n)*ht(n2,11)
       end if
      end if
...
C
 ...
 4010 FORMAT(' Simulation terminated')
      END
      DOUBLE PRECISION FUNCTION VTRHO(CC,I)
C*****
CVTRHO
C*****
C
C  DENSITY AS A FUNCTION OF TEMPERATURE CC
C
      include 'd_rprop.inc'
      IMPLICIT DOUBLE PRECISION (A-H,P-Z)
c      include 'c_rprop.inc'
c      VTRHO=HT(I,6)+HT(I,8)*(CC-HT(I,7))
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION THERMC(CC,I)
C*****
CTHERMC
C*****
C
C THERMAL CONDUCTIVITY AS FUNCTION OF THETA
C
      include 'd_rprop.inc'
      IMPLICIT DOUBLE PRECISION (A-H,P-Z)
      integer hydraulicFunctionType, adsorptionType
...
c      include 'c_rprop.inc'
      ...
      IF(CC.LE.T1) THEN
       THERMC=HT(I,9)
      ELSE
       IF(CC.GE.HK(I,3)) THEN
        THERMC=HT(I,10)
       ELSE
        W1=(CC-HK(I,5))/(HK(I,3)-T1)
        W2=1-W1
        THERMC=W1*HT(I,9)+W2*HT(I,10)
       END IF
      END IF
      RETURN
      END
...
C   VS2DH fortran program to VS2DHI post processor
C
      SUBROUTINE GETNX(NX)
